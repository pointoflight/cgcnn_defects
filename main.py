...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, normalizer, col_ign=args.ignore)

        # evaluate on validation set
        mae_error = validate(val_loader, model, criterion, normalizer, epoch, col_ign=args.ignore)
    ...
    validate(test_loader, model, criterion, normalizer, epoch, test=True, col_ign=args.ignore)
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer, col_ign=[]):
    ...
    for i, (input, target, _) in enumerate(train_loader):
        ...
        loss = lossfn(output, target_var, col_ign, criterion, "training", epoch)
...
def validate(val_loader, model, criterion, normalizer, epoch, test=False, col_ign=[]):
    batch_time = AverageMeter()
    losses = AverageMeter()
    if args.task and {'regression', 'r'}:
        mae_errors = AverageMeter()
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        ...
        # compute output
        output = model(*input_var)
        loss = lossfn(output, target_var, col_ign, criterion, "validation", epoch)
